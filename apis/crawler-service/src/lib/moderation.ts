...
export interface DomainReputation {
    ...
    flaggedCount?: number; // Optional: not in current schema
    lastUpdated: Date;
}
...
export class ContentModerationService {
    ...
    async detectSpam(url: string, title: string, description: string): Promise<SpamDetectionResult> {
        ...
        const domain = new URL(url).hostname.toLowerCase();
        const domainRep = await this.getDomainReputation(domain);
        if (domainRep && domainRep.score < 0.3) {
            reasons.push('low-reputation-domain');
            isSpam = true;
            confidence += 0.3;
        }
    ...
    async updateDomainReputation(domain: string, wasAccepted: boolean): Promise<void> {
        try {
            ...
            // Note: flagged_count column doesn't exist in current schema
            // Score is simply based on submission count for now
            const newScore = Math.min(1.0, 0.5 + (newSubmissionCount * 0.05)); // Increase trust with more submissions
