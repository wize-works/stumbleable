...
import { allowServiceToService, requireAdmin } from '../middleware/auth';
import type { JobRegistration } from '../types';

/**
 * Job Management API Routes
 * 
 * Authentication Strategy:
 * - /jobs/register: Service-to-service (no user auth required)
 * - All other endpoints: Admin-only (requires Clerk authentication + admin role)
 */
...
const RegisterJobSchema = z.object({
    ...
    enabled: z.boolean(),
    jobType: z.enum(['email', 'crawler', 'cleanup', 'analytics', 'other']),
...
export default async function jobRoutes(fastify: FastifyInstance) {
     ...
     * Register a new job from an external service
     * 
     * Authentication: Service-to-service (no user auth)
     */
    fastify.post<{
        Body: z.infer<typeof RegisterJobSchema>;
    }>('/jobs/register', {
        preHandler: allowServiceToService
    }, async (request, reply) => {
        try {
            const jobData = RegisterJobSchema.parse(request.body);

            const job: JobRegistration = {
                ...jobData,
            };

            // Add serviceUrl for internal use
            const serviceUrl = process.env[`${job.service.toUpperCase().replace(/-/g, '_')}_URL`] || 'http://localhost:8080';
            ...
            return reply.send({
                success: true,
                message: `Job ${job.name} registered successfully`,
            });
        } catch (error: any) {
    ...
    fastify.put<{
        ...
        try {
            ...
            await scheduler.updateCronExpression(jobName, cronExpression);
